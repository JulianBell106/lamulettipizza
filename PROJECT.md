...
> - **Rename WhatsApp sender display name to "Stalliq"** — name APPROVED by Meta 2026-06-06 ✅. Number re-registration needed to apply it; Twilio-managed sender so Twilio must do it — **support ticket submitted 2026-06-06, awaiting Twilio**. Verify number shows "Stalliq" (not "Endoo Limited") in Meta WhatsApp Manager once done.
...
> 2b. **Rename WhatsApp sender to "Stalliq"** — name approved by Meta 2026-06-06 ✅; Twilio support ticket submitted for number re-registration. ⏳ Awaiting Twilio, then verify in Meta WhatsApp Manager.
...
> - ⚠️ **Sender display name → "Stalliq" (in progress).** New name APPROVED by Meta 2026-06-06 ✅ but number still registered as "Endoo Limited" — name approval ≠ number registration. Twilio-managed sender, so only Twilio (BSP) can re-register; **Twilio support ticket submitted 2026-06-06**. Sender may go briefly Offline→Online during re-registration (harmless pre-launch — La Muletti go-live is August 2026). One shared number = one display name for all vendors; vendor still named in the body via `{{2}}`. True per-vendor sender names need a separate number/WABA per vendor — future premium option.
